;(function($){
    ...
    $.JRepeatable = function(input, options){
        ...
        self.init = function(){
            ...
            if($('body').find('>'+self.options.container).length){
            	self.$container_old = $(self.options.container).eq(0);
            	$('body').find('>'+self.options.container).find('.modal').html(self.$container_old.find('.modal').html())
				self.$container_old.remove();
        ...
        self.refreshValue = function(){
            ...
            $(self.$input.attr('data-input')).val(JSON.stringify(self.values));

            // tell all about value changed
            $(self.$input.attr('data-input')).trigger('value-update', self.values);
        };
